define(['backbone', 'backbone.marionette', 'underscore', 'audio', 'q'], function(Backbone, Marionette, _, Audio, Q) {
  ...
  var SamplePlayer = Backbone.Model.extend({
    ...
    initialize: function() {
      ...
      var url
      if (params.speech) {
        url = window.config.soundserver.url + "/speech_api?text="+encodeURIComponent(params.text)+"&pitch="+encodeURIComponent(params.pitch)+"&speed="+encodeURIComponent(params.speed)+"&voice="+encodeURIComponent(params.voice)+""
        ...
        url = window.config.soundserver.url + "/sounds/bank/" + encodeURIComponent(params.file)
